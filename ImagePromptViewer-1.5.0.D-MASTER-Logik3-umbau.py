...
def extract_text_chunks(img_path):
    ...
    # === Logik 5: Smart JPEG Marker Parsing (Fallback nur wenn alles andere fehlschlägt) ===
    try:
        ...
        idx3 = normalized_text.find(marker3, idx2)

        if idx1 != -1 and idx2 != -1 and idx3 != -1:
            prompt_block = clean_textblock(normalized_text[idx1 + len(marker1): idx2])
            negative_block = clean_textblock(normalized_text[idx2 + len(marker2): idx3])
            settings_block = normalized_text[idx3:].strip()
            debug_info.append("Debug: USING Logik 5 (Smart JPEG Fallback)")
            if hasattr(ImageManagerForm, 'instance'):
                ImageManagerForm.instance.debug_info = "\n".join(debug_info)
            return prompt_block, negative_block, settings_block
        else:
            debug_info.append("Debug: Logik 5 Marker nicht vollständig gefunden oder andere Logik hatte Vorrang.")
    except Exception as e:
