...
Änderungen in Version 1.0.31.12:
- Filter-History verbessert: Doppelte Einträge vermeiden, History nur bei tatsächlicher Anwendung aktualisieren.
- Menü-Interaktion für Filter-History optimiert: Korrekte Positionierung und Auswahl.
...
- Filter-Textfeld mit History der letzten 10 gefilterten Begriffe als Dropdown beim Klicken.
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def __init__(self):
        ...
        self.filter_entry.bind("<Return>", lambda e: self.apply_filter())  # Filter bei Enter anwenden
        self.filter_entry.bind("<Button-1>", self.show_filter_history)
    ...
    def show_filter_history(self, event):
        if not self.filter_history:
            return
        menu = tk.Menu(self, tearoff=0)
        for term in reversed(self.filter_history):  # Reversed, damit neuester Eintrag oben ist
            menu.add_command(label=term, command=lambda t=term: self.set_filter_from_history(t))
        # Positionierung des Menüs unter dem Filterfeld
        x = self.filter_entry.winfo_rootx()
        y = self.filter_entry.winfo_rooty() + self.filter_entry.winfo_height()
        menu.post(x, y)

    def set_filter_from_history(self, term):
