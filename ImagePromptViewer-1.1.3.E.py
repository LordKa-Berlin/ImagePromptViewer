...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def setup_ui(self):
        ...
        self.filter_button.place(x=10, y=10)  # Position kannst du später anpassen
    ...
    def apply_filters(self):
        """Filtert Bilder basierend auf Prompt-Stichwörtern + Einstellungen aus dem Filter-Settings-Formular."""
        import datetime

        keywords = self.filter_prompt.get().strip().lower().split(",") if hasattr(self, "filter_prompt") else []
        keywords = [kw.strip() for kw in keywords if kw.strip()]

        mode = self.prompt_filter_mode.get() if hasattr(self, "prompt_filter_mode") else "all"

        min_size = int(self.min_size_var.get()) if hasattr(self, "min_size_var") and self.min_size_var.get().isdigit() else None
        max_size = int(self.max_size_var.get()) if hasattr(self, "max_size_var") and self.max_size_var.get().isdigit() else None

        today = datetime.datetime.now()
        filter_age = None
        if hasattr(self, "date_one_week") and self.date_one_week.get():
            filter_age = today - datetime.timedelta(days=7)
        elif hasattr(self, "date_two_weeks") and self.date_two_weeks.get():
            filter_age = today - datetime.timedelta(days=14)
        elif hasattr(self, "date_one_month") and self.date_one_month.get():
            filter_age = today - datetime.timedelta(days=30)
        ...
        for img in self.folder_images:
            ...
            # 🔎 Prompt Text Prüfung
            prompt_text = self.prompt_dict.get(img, "").lower()

            if keywords:
                if mode == "all" and not all(k in prompt_text for k in keywords):
                    passed = False
                elif mode == "any" and not any(k in prompt_text for k in keywords):
                    passed = False
                elif mode == "none" and any(k in prompt_text for k in keywords):
                    passed = False

            # 📅 Datum prüfen
            if passed and filter_age:
                try:
                    file_date = datetime.datetime.fromtimestamp(os.path.getmtime(img))
                    if file_date < filter_age:
                        passed = False
                except:
                    pass

            # 📦 Dateigröße prüfen
            if passed and (min_size or max_size):
                try:
...
if __name__ == "__main__":
    ...
    def reset_all_filters(self):
        """Setzt alle Filter im Hauptfenster und in den Settings zurück."""
        if hasattr(self, "filter_prompt"):
            self.filter_prompt.set("")
        if hasattr(self, "filter_prompt_entry"):
            self.filter_prompt_entry.set("")
        ...
        for attr in ["date_one_week", "date_two_weeks", "date_one_month"]:
