...
"""
Datum: 2025-04-01
Versionsnummer: 1.2.1.b
Interne Bezeichnung: Master8 Alpha10

Änderungen in Version 1.2.1.b:
- Fix: In apply_filters() wurde der Fallback entfernt, der bei leerer Filterliste immer das erste Bild auswählte. Nun wird bei keinen Treffern auch kein Bild angezeigt.
- Anpassung: Das Filter Settings Fenster passt nun seine Größe dynamisch an die Monitorauflösung an (Referenz: 2550x1440). So werden auf hochauflösenden Monitoren (z.B. 3840x2160) auch alle Buttons sichtbar.

Zusammenfassung:
Ein Bildbetrachter für PNG- und JPEG-Dateien, der Textchunks auswertet (PNG: info['parameters'], JPEG: EXIF-Tag 37510) und in Prompt, Negative Prompt und Settings aufteilt. Neu: Erweiterte Filterfunktionen, inklusive verbesserter Datum-Filter und dynamisch skalierendem Filter Settings Fenster.
"""
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def setup_ui(self):
        ...
        visible_chars = max(20, int(50 * self.scaling_factor)) # Folder Path Combo Cox länge einstellen
    ...
    def open_filter_settings(self):
        ...
        # Dynamische Größeneinstellung des Filter Settings Fensters anhand der Monitorauflösung.
        base_width_fs = 400
        base_height_fs = 600
        # Referenz: 2550x1440 (korrekte Darstellung) -> Skalierungsfaktor
    ...
    def apply_filters(self):
        ...
        # NEUER BEHERRSCHUNG: Entferne den Fallback, der immer ein Bild auswählte, wenn kein Treffer gefunden wurde.
