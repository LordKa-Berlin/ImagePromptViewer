...
Änderungen in Version 1.0.31.24:
- Default-Bildgröße im Hauptformular angepasst, um dynamisch an die Monitorauflösung anzupassen:
...
- Funktion get_image_scale_factor entfernt, da sie nicht mehr benötigt wird.
...
def get_default_image_scale(scaling_factor):
    # Dynamischer Skalierungsfaktor für die Default-Bildgröße
    # Bei 3840x2160 (scaling_factor = 1): Entspricht "50%" (default_scale = 0.5)
    # Bei 1920x1080 (scaling_factor = 0.5): Entspricht "25%" (default_scale = 0.25)
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def display_image(self, file_path, default_scale=False):
        ...
        if default_scale or (self.scale_var.get() == "Default"):
            # Berechne den dynamischen Skalierungsfaktor für die Default-Einstellung
