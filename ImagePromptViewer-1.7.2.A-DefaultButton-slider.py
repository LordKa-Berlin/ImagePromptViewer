...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def setup_ui(self):
        ...
        self.image_frame.pack(fill="both", expand=True, padx=self.button_padding, pady=self.button_padding)
                ...
                # Neuer Frame für Slider – wird unter dem Bild und vor den Back/Next-Buttons platziert
        scale_frame = tk.Frame(right_controls, bg=BG_COLOR_Test)
        scale_frame.pack(fill="x", padx=self.button_padding, pady=self.button_padding)


            # Variablen für den Slider und die Checkbox
        self.scale_value = tk.IntVar(value=50)  # Standardwert: 50%
        self.default_scale_var = tk.BooleanVar(value=True)  # Default an

            # Slider selbst, zentriert
        self.scale_slider = tk.Scale(scale_frame, from_=10, to=100, orient="horizontal",
                 variable=self.scale_value,
                                command=lambda v: self.rescale_image(),
                                bg=BG_COLOR_Test, fg=TEXT_FG_COLOR,
                                highlightbackground=BG_COLOR_Test)
        self.scale_slider.pack(anchor="center", pady=2)
        ...
        # Label (in Englisch)
        scaling_label = tk.Label(scaling_frame, text="Image Display Scaling", bg=BG_COLOR, fg=TEXT_FG_COLOR, font=("Arial", self.main_font_size))
        scaling_label.pack(side="left", padx=(self.button_padding, 5))

        # Schieberegler: Werte von 10 bis 100 (%)
        self.scaling_slider = tk.Scale(scaling_frame, from_=10, to=100, orient="horizontal", bg=BG_COLOR, fg=TEXT_FG_COLOR, font=("Arial", self.main_font_size), command=self.on_scaling_slider_change)
        ...
        self.default_scaling_cb = tk.Checkbutton(scaling_frame, text="Default-2", variable=self.default_scaling_var, bg=BG_COLOR, fg=TEXT_FG_COLOR, selectcolor=BG_COLOR, font=("Arial", self.main_font_size), command=self.on_default_scaling_toggle)
        ...
        if len(self.monitor_list) > 1:
            ...
            self.monitor_menu = tk.OptionMenu(scale_frame, self.monitor_choice, *monitor_names,
