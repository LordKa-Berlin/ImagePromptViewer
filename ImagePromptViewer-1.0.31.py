...
class ImageManagerForm(TkinterDnD.Tk):
    def __init__(self):
        ...
        self.preview_images = {}  # Cache für Thumbnails
...
.monitor_list):
    ...
    def load_folder_async(self, folder, file_path=None):
        """Asynchrones Laden der Bilder aus einem Ordner."""
    ...
    def on_folder_loaded(self, folder, file_path):
        """Callback nach asynchronem Laden."""
        if self.folder_images:
            ...
            self.status(f"Ordner geladen: {folder} ({len(self.folder_images)} Bilder)")
        else:
    ...
    def populate_preview_table_lazy(self):
        """Lazy Loading der Vorschau: Nur sichtbare Elemente rendern."""
        for widget in self.preview_inner_frame.winfo_children():
            widget.destroy()
        self.preview_items = []
        # Platzhalter für alle Bilder erstellen
    ...
    def update_preview_visible(self):
        """Aktualisiert nur die sichtbaren Thumbnails."""
        ...
        visible_end = min(len(self.folder_images), int(y_bottom // (int(100 * self.scaling_factor) + 4)) + 1)

        for i in range(visible_start, visible_end):
            frame = self.preview_items[i]
            if not frame.winfo_children() or len(frame.winfo_children()) == 1:  # Nur Text vorhanden
                file_path = self.folder_images[i]
        ...
        if hasattr(self, "current_image_path") and self.current_image_path:
            ...
            if confirm:
                try:
                    ...
                    self.folder_images.pop(self.current_index)
                    self.populate_preview_table_lazy()
