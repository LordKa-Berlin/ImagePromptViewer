...
"""
Datum: 2025-03-25
Versionsnummer: 1.0.35.2
Interne Bezeichnung: Master 3 Alpha 1

Änderungen in Version 1.0.35.2:
- Fehlerbehebung bei der Extraktion von Textchunks aus JPEG-Dateien:
  - Korrigierte die Dekodierung von UTF-16-Texten (Entfernung von \x00-Zeichen).
  - Verbesserte die Erkennung von Trennzeichen (\nBREAK\n, Negative prompt:, Steps:).
  - Prompt, Negative Prompt und Settings werden nun korrekt extrahiert und angezeigt.
...
def extract_text_chunks(img_path):
    ...
    # Suche nach dem passenden Marker basierend auf dem Dateityp
    for key, value in info.items():
        if is_jpeg and "UNICODE" in str(value):
            param_key = key
            full_text = str(value)
            break
        elif not is_jpeg and "parameters" in key.lower():
            param_key = key
            full_text = str(value)
            break
    ...
    # Für JPEG-Dateien: Entferne \x00-Zeichen (UTF-16-Dekodierung)
    if is_jpeg:
        # Entferne \x00-Zeichen, die zwischen jedem Buchstaben stehen
    ...
    idx_break = full_text.find("\\nBREAK\\n", start)

    # Extrahiere den Prompt
    if idx_break != -1:
        # Wenn ein BREAK vorhanden ist, ist das der Hauptabschnitt für den Prompt
        if idx_neg != -1 and idx_neg < idx_break:
            prompt = full_text[start:idx_neg].strip()
        elif idx_steps != -1 and idx_steps < idx_break:
            prompt = full_text[start:idx_steps].strip()
        else:
            prompt = full_text[start:idx_break].strip()
    else:
        # Kein BREAK vorhanden, suche nach den anderen Trennzeichen
        if idx_neg != -1:
            prompt = full_text[start:idx_neg].strip()
        elif idx_steps != -1:
            prompt = full_text[start:idx_steps].strip()
    ...
    # Entferne zusätzliche JSON-Metadaten aus dem Prompt, falls vorhanden
    prompt = prompt.split('"extraMetadata":')[0].strip()
