...
"""
Datum: 2025-04-01
Versionsnummer: 1.2.1.c
Interne Bezeichnung: Master8 Alpha10

Änderungen in Version 1.2.1.c:
- Dynamische Anpassung der Fenstergröße: Das Hauptformular verwendet nun 90% der Monitorauflösung, sodass es auf Monitoren mit 3840x2160, 2560x1440 oder 1920x1080 vollständig sichtbar ist.
- Filter Combobox History: Bei Eingabe eines neuen Filtertexts wird dieser (falls noch nicht vorhanden) in einer History (max. 10 Einträge) gespeichert und in der Combobox angezeigt.
- Filter Button Farbänderung: Der Filter-Button ändert seine Hintergrundfarbe (auf Rot), wenn ein Filter aktiv ist (z. B. Filtertext vorhanden oder Datum-/File-Size‑Filter gesetzt sind).

Zusammenfassung:
Ein Bildbetrachter für PNG- und JPEG-Dateien, der Textchunks auswertet (PNG: info['parameters'], JPEG: EXIF-Tag 37510) und in Prompt, Negative Prompt und Settings aufteilt. Neu: Erweiterte Filterfunktionen, dynamisch skalierendes Hauptformular, Filter-History und visuelle Rückmeldung bei aktivem Filter.
"""
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def setup_ui(self):
        ...
        visible_chars = max(20, int(50 * self.scaling_factor))  # Hier wird die Combobox länger gemacht
    ...
    # --- Neue Methode: open_filter_settings korrekt innerhalb der Klasse definieren ---
    def open_filter_settings(self):
        ...
        base_width_fs = 400
        base_height_fs = 600
        fs_scaling_factor = self.selected_monitor.width / 2550
        fs_width = int(base_width_fs * fs_scaling_factor)
        fs_height = int(base_height_fs * fs_scaling_factor)
        self.filter_settings_window.geometry(f"{fs_width}x{fs_height}")

        prompt_frame = tk.LabelFrame(self.filter_settings_window, text="Prompt Filter", fg=TEXT_FG_COLOR, bg=BG_COLOR,
        ...
        date_frame = tk.LabelFrame(self.filter_settings_window, text="Date Filter", fg=TEXT_FG_COLOR, bg=BG_COLOR,
                                   font=("Arial", int(self.main_font_size * 1.2), "bold"))
        ...
        size_frame = tk.LabelFrame(self.filter_settings_window, text="File Size (KB)", fg=TEXT_FG_COLOR, bg=BG_COLOR,
                                   font=("Arial", int(self.main_font_size * 1.2), "bold"))
