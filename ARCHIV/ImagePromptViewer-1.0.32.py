...
Änderungen in Version 1.0.32:
- Korrektur der Funktion extract_png_text_chunks, um den Prompt korrekt nach "parameters" zu extrahieren, unabhängig von vorangestelltem Text oder Leerzeichen.
...
def extract_png_text_chunks(png_path):
    ...
    if idx_params == -1:
        # Falls "parameters" nicht gefunden wird, leer zurückgeben
        return "", "", ""
    # Startpunkt ist direkt nach "parameters"
    start = idx_params + len("parameters")
    # Suche nach "negative prompt:" als Endpunkt
    idx_neg = full_text.find("negative prompt:", start)
    if idx_neg == -1:
        # Wenn kein "negative prompt:" gefunden wird, alles bis zum Ende nehmen
        prompt = full_text[start:].strip()
        negativ = ""
        settings = ""
    else:
        prompt = full_text[start:idx_neg].strip()
        # Suche nach "steps:" für die Settings
        idx_steps = full_text.find("steps:", idx_neg)
        if idx_steps == -1:
