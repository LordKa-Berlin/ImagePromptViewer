...
Änderungen in Version 1.0.31.23:
- Dynamische Anpassung der Default-Bildgröße im Hauptformular basierend auf der Monitorauflösung:
  - Bei 3840x2160: +50% (max_size von 300 auf 450).
  - Bei 1920x1080: +25% (max_size von 150 auf 187.5).
  - Lineare Interpolation für dazwischenliegende Auflösungen.
- Schriftgröße auf 16 gesetzt (wie vom Benutzer angepasst).
...
def get_image_scale_factor(scaling_factor):
    # Dynamischer Skalierungsfaktor für die Bildgröße
    # Bei 3840x2160 (scaling_factor = 1): +50% (image_scale_factor = 1.5)
    # Bei 1920x1080 (scaling_factor = 0.5): +25% (image_scale_factor = 1.25)
    # Lineare Interpolation dazwischen
    image_scale_factor = 1.25 + 0.5 * (scaling_factor - 0.5)
    return max(1.25, min(1.5, image_scale_factor))

def get_font_size(monitor, base_size=16):  # Benutzeranpassung: 16
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def display_image(self, file_path, default_scale=False):
        ...
        if default_scale or (self.scale_var.get() == "Default"):
            ...
            # Basisgröße für das Bild (bei 3840x2160: 300)
            base_max_size = 300
            # Skaliere die Basisgröße mit dem allgemeinen Skalierungsfaktor
            max_size = int(base_max_size * self.scaling_factor)
            # Berechne den zusätzlichen Skalierungsfaktor für die Bildgröße
            image_scale_factor = get_image_scale_factor(self.scaling_factor)
            # Wende den zusätzlichen Skalierungsfaktor an
            max_size = int(max_size * image_scale_factor)
            if w >= h:
                new_width = max_size
