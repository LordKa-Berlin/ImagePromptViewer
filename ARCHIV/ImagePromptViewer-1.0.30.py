...
Änderungen in Version 1.0.31.6:
- Funktion extract_png_text_chunks angepasst, um Text präzise aufzuteilen:
  - Prompt: Text nach "parameters" bis "Negative prompt:".
  - Negativ Prompt: Text nach "Negative prompt:" bis "Steps:".
  - Settings: Text ab "Steps:" bis zum Ende.
- Sicherstellung, dass Textfelder im Hauptformular und in der Vollansicht korrekt gefüllt werden.
...
def extract_png_text_chunks(png_path):
    ...
    info = img.info
    # Suche nach einem Schlüssel, der "parameters" enthält (z. B. "tEXtparameters")
    param_key = None
    full_text = ""
    ...
    if not full_text:
        if hasattr(ImageManagerForm, 'instance'):
            ImageManagerForm.instance.status("Debug: Kein Schlüssel mit 'parameters' gefunden.")
        return "", "", ""
    
    # Debugging: Vollständigen Text aus dem Schlüssel ausgeben
    if hasattr(ImageManagerForm, 'instance'):
        ImageManagerForm.instance.status(f"Debug: Text aus {param_key}: {repr(full_text)[:100]}...")
    
    # Suche nach den Trennstellen (case-sensitive wie im Beispiel)
    ...
    # Debugging: Ausgabe der extrahierten Teile
    if hasattr(ImageManagerForm, 'instance'):
