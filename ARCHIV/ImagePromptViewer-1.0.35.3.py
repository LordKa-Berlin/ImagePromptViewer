...
"""
Datum: 2025-03-25
Versionsnummer: 1.0.35.3
Interne Bezeichnung: Master 3 Alpha 1

Änderungen in Version 1.0.35.3:
- Fehlerbehebung bei der Extraktion von Textchunks aus JPEG-Dateien:
  - Korrigierte die Dekodierung von UTF-16-Texten durch Verwendung von decode('utf-16').
  - Entfernte die falsche Verwendung von \nBREAK\n als Trennzeichen.
  - Verwende ausschließlich 'Negative prompt:' und 'Steps:' als Trennzeichen.
...
def extract_text_chunks(img_path):
    ...
    for key, value in info.items():
        if is_jpeg and "UNICODE" in str(value):
            param_key = key
            # Konvertiere den Wert in Bytes und dekodiere ihn als UTF-16
            if isinstance(value, bytes):
                full_text = value.decode('utf-16', errors='ignore')
            else:
                full_text = str(value).encode('utf-8').decode('utf-16', errors='ignore')
            break
