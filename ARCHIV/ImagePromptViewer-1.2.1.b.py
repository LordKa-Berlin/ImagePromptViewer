...
- Git-Anweisungen am Ende des Skripts wurden angepasst.
...
# ---------------------------------------------------------------------
# Restlicher Code (unverändert bis auf sichtbare Texte und Filterlogik)
# ---------------------------------------------------------------------
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def clear_filter_inputs(self):
        # Reset der Filter Settings Eingaben
    ...
    def reset_all_filters(self):
        # Reset alle Filter, auch die Filter Combo im Hauptformular
        self.filter_var.set("")
    ...
    def apply_filters(self):
        # Neue Filterlogik: Alle aktiven Filterkriterien müssen erfüllt sein.
        ...
        if not keywords:
            ...
            for file_path in self.folder_images:
                ...
                # Prompt filter (verwende den Modus aus Filter Settings, falls aktiv)
                if self.filter_prompt_var.get():
                    ...
                    if mode == "all":
                        ...
                        if any(keyword in prompt_lower for keyword in keywords):
                            passes = False
                # Filename-Filter (any logic)
                if passes and self.filter_filename_var.get():
                    if not any(keyword in filename for keyword in keywords):
                        passes = False
                # Negative Prompt-Filter (any logic)
                if passes and self.filter_negativ_var.get():
                    if not any(keyword in negativ.lower() for keyword in keywords):
                        passes = False
                # Settings-Filter (any logic)
                if passes and self.filter_settings_var.get():
                    if not any(keyword in settings.lower() for keyword in keywords):
                        passes = False
                # File Size Filter
                if passes and ((hasattr(self, "entry_min_size") and self.entry_min_size.get().strip()) or 
                    ...
                    if passes and self.entry_max_size.get().strip():
                        if file_size_kb > int(self.entry_max_size.get().strip()):
                            passes = False
                # Datumfilter: Platzhalter – hier kann später die Logik ergänzt werden.
                if passes:
    ...
    def apply_filter(self):
        # In diesem Fall wird apply_filters() aufgerufen, um alle Kriterien anzuwenden.
        self.apply_filters()
