...
def setup_ui(self):
    ...
    # Hauptcontainer: zwei Spalten (links: Filter Settings, rechts: bestehende Steuerelemente & Bildanzeige)
    ...
    # Between two dates (Checkbox + zwei Entry-Felder)
    ...
    # Not older than X days (Checkbox + Entry)
    ...
    # Older than X days (Checkbox + Entry)
    ...
    # (Optional: Entferne die alten vordefinierten Zeiträume wie "Created this week" etc. falls nicht mehr benötigt)

    # Zusätzlich: Weitere Filteroptionen bleiben (z.B. Prompt Filter, File Size Filter)
    ...
    self.filter_combo['values'] = self.filter_history_list  # History der Filter
    ...
    self.subfolder_cb = tk.Checkbutton(subfolder_frame, text="Search subfolders",
                                       ...
                                       fg=TEXT_FG_COLOR, bg=BG_COLOR, selectcolor=BG_COLOR,
                                       font=("Arial", self.main_font_size),
                                       command=lambda: threading.Thread(target=self.load_folder_async, 
                                                                          args=(self.folder_path_var.get(),),
                                                                          daemon=True).start() if self.folder_path_var.get() else None)
    ...
    self.drop_canvas.create_text(int(75 * self.scaling_factor), int(56 * self.scaling_factor),
                                  text="Drop Image Here", fill="white",
                                  font=("Arial", self.main_font_size))
