...
Änderungen in Version 1.0.31.2:
- Überarbeitung der Funktion extract_png_text_chunks, um sicherzustellen, dass "parameters" korrekt gefunden und extrahiert wird, mit Debugging-Informationen.
...
def extract_png_text_chunks(png_path):
    ...
    info = img.info
    # Den gesamten Text aus allen Info-Werten zusammenfügen
    full_text = " ".join([str(v) for v in info.values()])
    
    # Debugging: Vollständigen Text ausgeben
    if hasattr(ImageManagerForm, 'instance'):
        ImageManagerForm.instance.status(f"Debug: Full text from info: {repr(full_text)[:100]}...")
    
    # Suche nach "parameters" im Text
    idx_params = full_text.find("parameters")
    if idx_params == -1:
        if hasattr(ImageManagerForm, 'instance'):
            ImageManagerForm.instance.status("Debug: 'parameters' nicht gefunden.")
        return "", "", ""
    
    # Startpunkt ist direkt nach "parameters"
    start = idx_params + len("parameters")
    
    # Suche nach "negative prompt:" und "steps:"
    ...
    if idx_neg != -1:
        ...
        prompt = full_text[start:].strip() if start < len(full_text) else ""
        negativ = ""
        settings = ""
        if hasattr(ImageManagerForm, 'instance'):
            ImageManagerForm.instance.status(f"Debug: Prompt extrahiert: {prompt[:50]}...")
    ...
    if idx_steps != -1 and idx_steps > idx_neg:
        ...
        settings = ""
        if hasattr(ImageManagerForm, 'instance'):
            ImageManagerForm.instance.status(f"Debug: Prompt: {prompt[:50]}..., Negativ: {negativ[:50]}...")
        return prompt, negativ, settings
    ...
    if hasattr(ImageManagerForm, 'instance'):
        ImageManagerForm.instance.status(f"Debug: Prompt: {prompt[:50]}..., Negativ: {negativ[:50]}..., Settings: {settings[:50]}...")
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def show_fullscreen(self):
        ...
        self.fullscreen_win.bind("<Delete>", self.handle_delete_key)
