...
"""
Datum: 2025-03-25
Versionsnummer: 1.0.35.6
Interne Bezeichnung: Master 8

Änderungen in Version 1.0.35.6:
- Fehlerbehebung bei der Extraktion von Textchunks aus JPEG-Dateien:
  - Korrigierte die Dekodierung von EXIF-Daten: Entferne das "UNICODE"-Präfix und dekodiere den Rest als UTF-16LE.
  - Fallback-Methode: Entferne \x00-Zeichen manuell und dekodiere als `latin-1`, wenn die Dekodierung fehlschlägt.
- Vorherige Änderungen (aus 1.0.35.5):
  - Dekodierung der EXIF-Daten mit ANSI (`cp1252`).
- Vorherige Änderungen (aus 1.0.35.4):
  - Verwende die Bibliothek `piexif` für eine direktere Extraktion der EXIF-Daten.
  - Dynamische Bestimmung der Kodierung (versuche `utf-16`, `utf-8`, `latin-1`).
- Vorherige Änderungen (aus 1.0.35.1):
  - Unterstützung für JPEG-Dateien hinzugefügt.
  - Dynamische Skalierung im Hauptformular angepasst.
- Vorherige Änderungen (aus 1.0.31.27):
  - Monitor-Pulldown-Button angepasst.
  - Default-Bildgröße im Hauptformular angepasst.
  - Filter-Kontrollkästchen angepasst ("Prompt"-Checkbox standardmäßig aktiviert).
  - Sortieroption angepasst (ASC/DESC-Button, Default: DESC).
  - Bildanzahl im Hauptformular angepasst (gefilterte Bildanzahl in Rot).
  - Caching für Erstellungsdaten und Textchunks hinzugefügt.
"""
...
def extract_text_chunks(img_path):
    ...
    if is_jpeg:
        # Verwende piexif, um EXIF-Daten direkt auszulesen
        try:
            exif_dict = piexif.load(img_path)
            # Suche in den EXIF-Daten nach einem Eintrag, der "UNICODE" enthält
            for ifd in exif_dict:
                for tag in exif_dict[ifd]:
                    value = exif_dict[ifd][tag]
                    if isinstance(value, bytes) and b"UNICODE" in value:
                        # Prüfe das Präfix "UNICODE" und dekodiere den Rest als UTF-16LE
                        if value.startswith(b"UNICODE\x00\x00"):
                            # Überspringe die ersten 8 Bytes ("UNICODE" + \x00)
                            text_bytes = value[8:]
                            try:
                                full_text = text_bytes.decode('utf-16le', errors='ignore')
                            except UnicodeDecodeError:
                                # Fallback: Entferne \x00-Zeichen und dekodiere als latin-1
                                full_text = text_bytes.replace(b'\x00', b'').decode('latin-1', errors='ignore')
                        else:
                            # Fallback: Dekodiere als cp1252
                            try:
                                full_text = value.decode('cp1252', errors='ignore')
                            except UnicodeDecodeError:
                                full_text = value.replace(b'\x00', b'').decode('latin-1', errors='ignore')
                        param_key = f"EXIF-{ifd}-{tag}"
                        break
                if full_text:
                    break
            if not full_text:
                # Fallback: Verwende img.info
                for key, value in info.items():
                    if "UNICODE" in str(value):
                        param_key = key
            ...
            if hasattr(ImageManagerForm, 'instance'):
                ImageManagerForm.instance.debug_info = f"Debug: Fehler beim Auslesen der EXIF-Daten: {e}"
            # Fallback: Verwende img.info
            for key, value in info.items():
                if "UNICODE" in str(value):
                    param_key = key
    ...
    if not full_text:
        if hasattr(ImageManagerForm, 'instance'):
            ImageManagerForm.instance.debug_info = f"Debug: Kein Schlüssel mit {'UNICODE' if is_jpeg else 'parameters'} gefunden."
        return "", "", ""
    
    debug_info = f"Debug: Text aus {param_key}: {repr(full_text)[:100]}...\n"
    
    # Bestimme den Startpunkt basierend auf dem Dateityp
    marker = "UNICODE" if is_jpeg else "parameters"
    idx_marker = full_text.find(marker)
    start = idx_marker + len(marker) if idx_marker != -1 else 0

    # Suche nach Trennzeichen
    idx_neg = full_text.find("Negative prompt:", start)
    idx_steps = full_text.find("Steps:", start)

    # Extrahiere den Prompt
    if idx_neg != -1:
    ...
    # Extrahiere den Negativen Prompt
    if idx_neg != -1:
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def __init__(self):
        ...
        self.sort_order = "DESC"  # Default: Neuestes Bild zuerst (absteigend)
        self.preview_images = {}
        self.fullscreen_win = None
        self.debug_info = ""
        self.filter_history = deque(maxlen=10)

        # Caches für Performance-Optimierung
        self.ctime_cache = {}  # Cache für Erstellungsdaten
        self.text_chunks_cache = {}  # Cache für Textchunks
    ...
    def setup_ui(self):
        ...
        # Frame für Bildanzahl mit zwei Labels
    ...
    def toggle_sort_order(self):
        if self.sort_order == "ASC":
            self.sort_order = "DESC"  # Wechsel zu absteigend (neuestes Bild zuerst)
            self.sort_button.config(text="ASC")
        else:
            self.sort_order = "ASC"  # Wechsel zu aufsteigend (ältestes Bild zuerst)
            self.sort_button.config(text="DESC")
        
        if self.folder_images:
            # Sortiere die Bildliste neu
            self.folder_images.sort(key=lambda x: self.ctime_cache[x], reverse=(self.sort_order == "DESC"))
            # Sortiere auch die gefilterte Liste neu, falls ein Filter aktiv ist
            if self.filtered_images:
                self.filtered_images.sort(key=lambda x: self.ctime_cache[x], reverse=(self.sort_order == "DESC"))
            # Zeige das erste Bild der neuen Sortierung an
            if self.filtered_images:
    ...
    def apply_filter(self):
        ...
        if not filter_text:
            ...
            for file_path in self.folder_images:
                ...
                # Verwende den Cache für Textchunks
        ...
        # Sortiere die gefilterte Liste entsprechend der aktuellen Sortierung
        if self.filtered_images:
            self.filtered_images.sort(key=lambda x: self.ctime_cache[x], reverse=(self.sort_order == "DESC"))
    ...
    def load_folder_async(self, folder, file_path=None):
        ...
        self.folder_images = []
        # Leere die Caches, da ein neuer Ordner geladen wird
        ...
        # Fülle den Cache für Erstellungsdaten
        for img_path in self.folder_images:
            self.ctime_cache[img_path] = os.path.getctime(img_path)
        
        # Sortiere Bilder nach Erstellungsdatum basierend auf sort_order
        self.folder_images.sort(key=lambda x: self.ctime_cache[x], reverse=(self.sort_order == "DESC"))
    ...
    def show_info(self):
        ...
        info_text = (
            ...
            "Liest PNG- und JPEG-Bilder (info['parameters'] für PNG, 'UNICODE' für JPEG) und teilt den Text in Prompt, Negativ, Settings.\n"
    ...
    def display_image(self, file_path, default_scale=False):
        ...
        # Berechne die verfügbare Größe des Anzeigebereichs
        self.image_frame.update_idletasks()  # Stelle sicher, dass die Größe aktuell ist
        ...
        if avail_width < 10 or avail_height < 10:
            # Falls der Frame noch nicht vollständig initialisiert ist, setze Standardwerte
            avail_width = 800
            avail_height = 600

        orig_width, orig_height = self.current_image.size

        # Berechne den Skalierungsfaktor basierend auf der Monitorauflösung
        ...
        # Begrenze die Größe auf 80 % des verfügbaren Anzeigebereichs
        max_width = int(avail_width * 0.8)
        max_height = int(avail_height * 0.8)

        # Berechne den Faktor, um das Bild in den Anzeigebereich zu passen
        width_factor = max_width / orig_width
        height_factor = max_height / orig_height
        fit_factor = min(width_factor, height_factor)

        # Wende den kleineren Faktor an, um das Bild in den Anzeigebereich zu passen
    ...
    def delete_current_image(self):
        ...
        if self.delete_immediately_main_var.get():
            try:
                ...
                # Entferne das Bild aus den Caches
