...
def extract_text_chunks(img_path):
    ...
    try:
        ...
        # Direkt nach den Schlüsseln suchen:
        prompt_json = data_dict.get("prompt", "")
        negative_json = data_dict.get("negativePrompt", "")
        # Normalerweise liefert "steps" nur einen Wert (z.B. 25).
        # Um den kompletten Textblock (inklusive des Markers "steps":) zu erhalten,
        # suchen wir im normalisierten Text nach dem Marker.
        ...
        # Falls diese direkten Schlüssel nicht gefunden wurden, versuche verschachtelte Suche:
        if not (prompt_json or negative_json or settings_text):
            for key, value in data_dict.items():
                if isinstance(value, dict):
                    ct = value.get("class_type", "").lower()
                    inputs = value.get("inputs", {})
                    if ct == "cliptextencode" and isinstance(inputs, dict) and "text" in inputs:
                        # Der erste CLIPTextEncode-Eintrag wird als Prompt verwendet,
                        # der zweite als Negative Prompt.
                        if not prompt_json:
                            prompt_json = inputs.get("text", "")
                        elif not negative_json:
                            negative_json = inputs.get("text", "")
                    elif ct == "ksampler" and isinstance(inputs, dict) and "steps" in inputs:
                        # Hier ebenfalls: den gesamten Textblock ab "steps": erzeugen
                        settings_text = '"steps":' + str(inputs.get("steps", ""))
                        break
        if prompt_json or negative_json or settings_text:
            debug_info.append("Debug: JSON-Parsierung erfolgreich (nested).")
