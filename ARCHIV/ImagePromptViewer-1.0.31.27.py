...
"""
Datum: 2025-03-25
Versionsnummer: 1.0.31.27
Interne Bezeichnung: Master 8

Änderungen in Version 1.0.31.27:
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def toggle_sort_order(self):
        ...
        if self.folder_images:
            ...
            self.folder_images.sort(key=lambda x: os.path.getctime(x), reverse=(self.sort_order == "DESC"))
            # Sortiere auch die gefilterte Liste neu, falls ein Filter aktiv ist
            if self.filtered_images:
                self.filtered_images.sort(key=lambda x: os.path.getctime(x), reverse=(self.sort_order == "DESC"))
