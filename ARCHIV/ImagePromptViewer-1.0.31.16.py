...
Änderungen in Version 1.0.31.18:
- Startgröße des Fensters um 10% reduziert (von 1200x1120 auf 1080x1008).
...
- Skalierungsfaktor basierend auf einer Basisauflösung von 1920x1080 angepasst, um auf kleineren Auflösungen besser zu skalieren.
...
def get_scaling_factor(monitor):
    # Basisauflösung: 1920x1080 (Full HD)
    base_width, base_height = 1920, 1080
    # Berechne Skalierung basierend auf der kleineren Dimension (Breite oder Höhe)
    width_factor = monitor.width / base_width
    height_factor = monitor.height / base_height
...
def get_window_size(monitor):
    factor = get_scaling_factor(monitor)
    # Basisgröße um 20% reduziert: 1200x1120 -> 960x896
    base_width, base_height = 840,1000
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def update_ui(self):
        ...
        if hasattr(self, 'folder_entry'):
            self.folder_entry.config(font=("Arial", self.main_font_size))
        ...
        if hasattr(self, 'scale_dropdown'):
            self.scale_dropdown.config(font=("Arial", self.main_font_size + 2))
        if hasattr(self, 'fullscreen_button'):
            self.fullscreen_button.config(font=("Arial", self.main_font_size))
        if hasattr(self, 'monitor_menu'):
            self.monitor_menu.config(font=("Arial", self.main_font_size + 2))
