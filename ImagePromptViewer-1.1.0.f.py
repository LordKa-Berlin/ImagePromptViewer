...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def __init__(self):
            ...
            # Setup UI vor History-Ladung
            self.setup_ui()
            self.update_scaling()

            # History nach setup_ui laden und Combobox-Werte setzen
            history_data = load_history()
            self.folder_history = history_data.get("folder_history", [])
            self.filter_history_list = history_data.get("filter_history", [])
            self.filter_combo['values'] = self.filter_history_list  # Jetzt sicher gesetzt
