...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def __init__(self):
        ...
        self.update_interval = 200  # Millisekunden
    ...
    def debounce_update_fs_image(self):
        current_time = time.time() * 1000  # In Millisekunden
        if current_time - self.last_update_time >= self.update_interval and not self.update_pending:
            self.update_fs_image()  # Korrigierter Aufruf
            self.last_update_time = current_time
        elif not self.update_pending:
            self.update_pending = True
            self.fullscreen_win.after(self.update_interval, self.process_pending_update)

    def process_pending_update(self):
        if self.update_pending:
            self.debounce_update_fs_image()
    ...
    def update_fs_image(self):
        try:
            ...
            if avail_w < 10 or avail_h < 10:
                self.fullscreen_win.after(100, self.debounce_update_fs_image)
                return
