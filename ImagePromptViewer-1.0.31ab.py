...
- Implementierung von Multithreading und Lazy Loading zur Optimierung des Ordner-Einlesens.
"""
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def load_folder_async(self, folder, file_path=None):
        ...
        if self.search_subfolders_var.get():
            ...
            self.folderistics = [os.path.join(folder, f) for f in os.listdir(folder) if f.lower().endswith(".png")]
    ...
    def populate_preview_table_lazy(self):
        """Lazy Loading der Vorschалеж: Nur sichtbare Elemente rendern."""
    ...
    def update_fs_texts(self):
        ...
        fs_prompt_text.config(state=tk.DISABLED)
        fs_prompt_text.pack(fill="x", padx=self.button_padding, pady=self.button_padding)
        fs_negativ_text = ScrolledText(self.fs_text_frame, height=8, bg=TEXT_BG_COLOR,
                                       fg=TEXT_FG_COLOR, font=("Arial", self.main_font_size))
        fs_negativ_text.insert("1.0", negativ)
        fs_negativ_text.config(state=tk.DISABLED)
        fs_negativ_text.pack(fill="x", padx=self.button_padding, pady=self.button_padding)
        fs_settings_text = ScrolledText(self.fs_text_frame, height=4, bg=TEXT_BG_COLOR,
                                        fg=TEXT_FG_COLOR, font=("Arial", self.main_font_size))
        fs_settings_text.insert("1.0", settings)
        fs_settings_text.config(state=tk.DISABLED)
        fs_settings_text.pack(fill="x", padx=self.button_padding, pady=self.button_padding)
