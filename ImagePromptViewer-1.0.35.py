...
"""
Datum: 2025-03-25
Versionsnummer: 1.0.35
Interne Bezeichnung: Master 3 Alpha 1

Änderungen in Version 1.0.35:
- Monitor-Pulldown-Button angepasst:
  - Wird nur in Multi-Monitor-Umgebungen angezeigt.
  - Zeigt die Auflösungen der angeschlossenen Monitore an.
  - Ermöglicht die Auswahl des Monitors für die Vollbildansicht (show_fullscreen).
- Default-Bildgröße im Hauptformular angepasst:
  - Bei 3840x2160: Entspricht der "50%"-Einstellung.
  - Bei 1920x1080: Entspricht der "25%"-Einstellung.
  - Lineare Interpolation für dazwischenliegende Auflösungen.
- Filter-Kontrollkästchen angepasst:
  - "Prompt"-Checkbox standardmäßig aktiviert.
- Sortieroption angepasst:
  - Checkbox durch Button ersetzt, der zwischen "ASC" (aufsteigend) und "DESC" (absteigend) wechselt.
  - Default: "DESC" (neuestes Bild zuerst).
  - Sortierung wird beim Ordner einlesen berücksichtigt.
- Bildanzahl im Hauptformular angepasst:
  - Anzeige: "Ordner: [Bildanzahl] Bilder gefiltert [Bildanzahl] Bilder".
  - Gefilterte Bildanzahl in Rot.
- Fehlerbehebung:
  - Bildaktualisierung bei Sortierwechsel korrigiert.
  - Bildanzahl-Anzeige korrigiert (kein tag_configure auf Label).
- Caching hinzugefügt:
  - Cache für Erstellungsdaten und Textchunks, um die Performance bei großen Ordnern zu verbessern.
"""
...
class ImageManagerForm(TkinterDnD.Tk):
        ...
        if not filter_text:
            ...
            for file_path in self.folder_images:
                ...
                if file_path not in self.text_chunks_cache:
                    self.text_chunks_cache[file_path] = extract_png_text_chunks(file_path)
    ...
    def display_image(self, file_path, default_scale=False):
        ...
        if default_scale or (self.scale_var.get() == "Default"):
            ...
            new_width = int(self.current_image.width * default_scale_factor)
            new_height = int(self.current_image.height * default_scale_factor)
        else:
    ...
    def update_fs_texts(self):
        ...
        if self.fs_image_path not in self.text_chunks_cache:
            self.text_chunks_cache[self.fs_image_path] = extract_png_text_chunks(self.fs_image_path)
