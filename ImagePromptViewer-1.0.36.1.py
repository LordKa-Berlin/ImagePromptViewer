...
"""
Datum: 2025-03-27
Versionsnummer: 1.0.36.1
Interne Bezeichnung: Master8 Alpha4

Änderungen in Version 1.0.36.1:
...
- Integration der neuen Routine zur Extraktion von Prompt-Daten aus JPEG-Dateien (PromptSlicer).
...
# ---------------------------
# NEUE EXTRAKTIONS-Routine (PromptSlicer)
# ---------------------------
def extract_text_chunks(img_path):
    """
    Extrahiert den Textchunk aus einem Bild.
    Für PNG: Sucht in info['parameters'].
    Für JPEG: Liest EXIF-Daten (UserComment, Tag 37510).
    
    Bei JPEG:
      - Falls das UserComment mit "UNICODE\x00\x00" beginnt, wird alles nach "UNICODE" verwendet.
      - Der resultierende Text wird normalisiert (alle Leerzeichenfolgen werden zu einem einzelnen Space).
      - Anschließend wird anhand der Marker "Negative prompt:" und "Steps:" aufgeteilt:
          * Prompt: Alles bis zum Marker "Negative prompt:" (Marker nicht übernommen).
          * Negativer Prompt: Alles zwischen dem Ende von "Negative prompt:" und dem Beginn von "Steps:" (Marker nicht übernommen).
          * Settings: Alles ab "Steps:" (inklusive).
    """
    # Öffne das Bild
    try:
        ...
        try:
            ...
            if user_comment and isinstance(user_comment, bytes):
                ...
                if user_comment.startswith(b"UNICODE\x00\x00"):
                    # Nutze piexif.helper um den Text zu laden, oder dekodiere ab Byte 8
                    try:
                    ...
                    debug_info.append(f"Debug: Kein UNICODE-Präfix, dekodiert als Latin-1.")
    ...
    if idx_neg != -1:
        # Negativer Prompt: Alles nach dem Marker "Negative prompt:" bis vor "Steps:" (Marker nicht enthalten)
