...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def setup_ui(self):
        ...
        # Filter Settings Button (angepasst, nicht entfernen)
    ...
    def apply_filters(self):
        ...
        def update_filter_button_state(self):
            """Passt die Farbe des Filter-Buttons an, wenn ein Filter aktiv ist."""
            filter_active = False
            ...
            # Checkboxen prüfen
            for var in ["filter_filename_cb", "filter_prompt_cb", "filter_negativ_cb", "filter_settings_cb"]:
                if hasattr(self, var) and getattr(self, var).get():
                    filter_active = True

            # Dateigröße prüfen
            if hasattr(self, "min_size_var") and self.min_size_var.get().strip():
                filter_active = True
            if hasattr(self, "max_size_var") and self.max_size_var.get().strip():
                filter_active = True


            # Filtermode (aus Settings-Fenster)
            if hasattr(self, "prompt_filter_mode") and self.prompt_filter_mode.get() != "none":
                filter_active = True

            # Farbe setzen
            if hasattr(self, "filter_button"):
                if filter_active:
                    self.filter_button.config(bg="#FF8800")  # z.B. Orange bei aktiven Filtern
                else:
                    self.filter_button.config(bg=BTN_BG_COLOR)  # Standardfarbe zurücksetzen
