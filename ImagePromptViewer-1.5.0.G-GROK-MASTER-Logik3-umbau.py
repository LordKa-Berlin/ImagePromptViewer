...
def extract_text_chunks(img_path):
        ...
        if marker1 in original_text and marker2 in original_text:
            ...
            if start_prompt != -1 and end_prompt != -1:
                ...
                if pos_steps != -1:
                    ...
                    # Erfolg nur, wenn beide Felder nicht leer sind
                    success = prompt_text and negative_text
                    if success:
                        debug_info.append("Debug: USING Logik 4 extraction (SUCCESS)")
                        if hasattr(ImageManagerForm, 'instance'):
                            ImageManagerForm.instance.debug_info = "\n".join(debug_info)
                        return prompt_text, negative_text, settings_text
                    else:
                        debug_info.append("Debug: Logik 4 - Not successful (empty fields found)")
                else:
    ...
    try:
        ...
        if matches:
            ...
            # Erfolg nur, wenn beide Felder nicht leer sind
            success = prompt_regex and negativ_regex
            if success:
                settings_regex = ""  # Settings bleibt leer
                debug_info.append("Debug: USING ComfyUI marker extraction (SUCCESS)")
                ...
                debug_info.append("Debug: ComfyUI markers found but not all fields have content")
        else:
            debug_info.append("Debug: No ComfyUI markers found")
    except Exception as e:
    ...
    try:
        ...
        if "models" in data_dict and isinstance(data_dict["models"], list):
            ...
            # Erfolg nur, wenn beide Felder nicht leer sind
            success = prompt_json and negative_json
            if success:
                ...
                debug_info.append("Debug: USING JSON parsing (models) (SUCCESS)")
                if hasattr(ImageManagerForm, 'instance'):
                    ImageManagerForm.instance.debug_info = "\n".join(debug_info)
                return str(prompt_json), str(negative_json), steps_json
            else:
                debug_info.append("Debug: JSON parsing (models) - not all fields have content")
        ...
        if not (prompt_json and negative_json):
        ...
        if success:
            ...
            debug_info.append("Debug: USING JSON parsing (direct) (SUCCESS)")
            if hasattr(ImageManagerForm, 'instance'):
                ImageManagerForm.instance.debug_info = "\n".join(debug_info)
            return str(prompt_json), str(negative_json), settings_text
        else:
            debug_info.append("Debug: JSON parsing (direct) - not all fields have content")
    except Exception as e:
    ...
    if ('"prompt":' in normalized_lower and 
        ...
        # Erfolg nur, wenn beide Felder nicht leer sind
        success = prompt_new and negativ_new
        if success:
            debug_info.extend([
            ...
            debug_info.append("Debug: USING New Markers extraction (SUCCESS)")
            if hasattr(ImageManagerForm, 'instance'):
                ImageManagerForm.instance.debug_info = "\n".join(debug_info)
            return prompt_new, negativ_new, settings_new
        else:
            debug_info.append("Debug: New Markers - not all fields have content")
    else:
        debug_info.append("Debug: New Markers not found in text")
    ...
    # Erfolg nur, wenn beide Felder nicht leer sind oder Fallback auf kompletten Text
    success = (prompt and negativ)
    if success:
        debug_info.extend([
        ...
        debug_info.append("Debug: USING Old Markers extraction (SUCCESS)")
    else:
        # Als letzten Fallback: Falls kein Prompt-Paar erfolgreich extrahiert werden konnte,
        # setze den gesamten Text als Prompt und lasse Negative leer
        prompt = normalized
        negativ = ""
        settings = ""
        debug_info.append("Debug: No successful extraction, using entire text as prompt (FALLBACK)")
