...
def extract_text_chunks(img_path):
    ...
    # === Logik 5: Smart JPEG Marker Parsing (Teilweise Fallback erlaubt) ===
    try:
        ...
        if prompt_block or negative_block or settings_block:
            debug_info.append("Debug: USING Logik 5 (Smart JPEG Fallback)")
            debug_info.append(f"Debug: Prompt: {repr(prompt_block)[:50]}...")
            debug_info.append(f"Debug: Negative Prompt: {repr(negative_block)[:50]}...")
            debug_info.append(f"Debug: Settings: {repr(settings_block)[:50]}...")
            if hasattr(ImageManagerForm, 'instance'):
                ImageManagerForm.instance.debug_info = "\n".join(debug_info)
            return prompt_block, negative_block, settings_block
        else:
