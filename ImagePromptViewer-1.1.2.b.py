...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def setup_ui(self):
        ...
        self.scaling_factor = self.winfo_fpixels('1i') / 72

        # === 📁 Ordnerpfad und Auswahl ===
        ...
        self.select_folder_button = tk.Button(folder_frame, text="Select folder",
        ...
        self.filter_prompt = tk.StringVar()
        ...
        self.filter_prompt_entry.bind("<Return>", lambda e: self.apply_filters())

        self.filter_button = tk.Button(filter_frame, text="Filter", command=self.apply_filters,
                                    bg=BTN_BG_COLOR, fg=BTN_FG_COLOR,
                                    font=("Arial", self.main_font_size))
        ...
        self.filter_settings_button = tk.Button(filter_frame, text="Filter Settings",
                                                command=self.open_filter_settings,
                                                bg=BTN_BG_COLOR, fg=BTN_FG_COLOR,
                                                font=("Arial", self.main_font_size),
                                                width=int(12 * self.scaling_factor))  # Dynamisch
        self.filter_settings_button.pack(side="left", padx=self.button_padding)

        self.reset_all_button = tk.Button(filter_frame, text="Reset All",
                                        command=self.reset_all_filters,
                                        bg=BTN_BG_COLOR, fg=BTN_FG_COLOR,
                                        font=("Arial", self.main_font_size),
                                        width=int(9 * self.scaling_factor))  # Dynamisch
        self.reset_all_button.pack(side="left", padx=self.button_padding)
        ...
        self.folder_path_combo = ttk.Combobox(folder_frame, textvariable=self.folder_path_var, width=60, font=("Arial", self.main_font_size))
    ...
    def open_filter_settings(self):
            ...
            # Radiobutton-Optionen für Prompt-Filter
            self.prompt_mode = tk.StringVar(value="")  # Leerer Startwert

            tk.Label(prompt_frame, text="Prompt Filtermodus:", bg=BG_COLOR, fg=TEXT_FG_COLOR, font=normal_font).pack(anchor="w", padx=10)

            modes = [
                ("All words must match", "all"),
                ("Any word", "any"),
                ("Exclude word(s)", "exclude"),
                ("None of the words", "none")
            ]

            for label, value in modes:
                tk.Radiobutton(prompt_frame, text=label, variable=self.prompt_mode, value=value,
                            bg=BG_COLOR, fg=TEXT_FG_COLOR, font=normal_font, selectcolor=BG_COLOR).pack(anchor="w", padx=20)
    ...
    def reset_all_filters(self):
        ...
        # 🔁 Alle Inputs im Filter-Settings-Fenster zurücksetzen (falls offen)
        self.clear_filter_inputs()

        # 🔡 Filter-Eingabefelder im Hauptformular leeren
        ...
        # 🎯 Radiobutton-Auswahl im Filter-Settings-Fenster zurücksetzen
        if hasattr(self, "prompt_mode"):
            self.prompt_mode.set("")
    ...
    def clear_prompt_filter(self):
        """Löscht nur das Prompt-Filterfeld im Hauptformular"""
        if hasattr(self, "filter_prompt_entry"):
            self.filter_prompt_entry.set("") if isinstance(self.filter_prompt_entry, ttk.Combobox) else self.filter_prompt_entry.delete(0, tk.END)
        if hasattr(self, "filter_prompt"):
            self.filter_prompt.set("")

                    
    def apply_filters(self):
            """
            Führt die kombinierte Filterung basierend auf:
            - Prompt-Eingabefeld im Hauptformular
            - gewählter Radiobutton-Option aus Filter Settings
            - Datei-Datum (z. B. letzte Woche)
            - Datei-Größe
            """
            ...
            prompt_input = self.filter_prompt.get().strip().lower() if hasattr(self, "filter_prompt") else ""
            keyword_list = [k.strip() for k in prompt_input.split(",") if k.strip()]
            mode = self.prompt_mode.get() if hasattr(self, "prompt_mode") else ""  # "all", "any", "exclude", "none"

            try:
                min_size = int(self.min_size_var.get()) if self.min_size_var.get() else None
            except:
                min_size = None
            ...
            for image_path in self.folder_images:
                ...
                if date_checks["this_week"] and (today - modified_time).days > 7:
                    passed = False
                if date_checks["two_weeks"] and (today - modified_time).days > 14:
                    passed = False
                if date_checks["four_weeks"] and (today - modified_time).days > 28:
                    passed = False
                if date_checks["one_month"] and (today - modified_time).days > 31:
                    passed = False
                if date_checks["one_year"] and (today - modified_time).days > 365:
                    passed = False
                ...
                if passed and keyword_list:
                    ...
                    if mode == "all":
                        if not all(kw in prompt_text for kw in keyword_list):
                            passed = False
                    elif mode == "any":
                        if not any(kw in prompt_text for kw in keyword_list):
                            passed = False
                    elif mode == "exclude":
                        if any(kw in prompt_text for kw in keyword_list):
                            passed = False
                    elif mode == "none":
                        if any(kw in prompt_text for kw in keyword_list):
                            passed = False
