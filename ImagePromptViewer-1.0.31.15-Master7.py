...
"""
Datum: 2025-03-25
Versionsnummer: 1.0.31.15
Interne Bezeichnung: Master7
...
Änderungen in Version 1.0.31.15:
...
- Debugging-Informationen hinzugefügt, um Pfadvergleiche zu überprüfen.
- Filter-Logik angepasst, um current_index nicht zu überschreiben, wenn das ausgewählte Bild bereits angezeigt wurde.
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def __init__(self):
        ...
        tk.Button(filter_frame, text="Filter", command=self.apply_filter,
                  bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=("Arial", self.main_font_size), width=6).pack(side="left", padx=self.button_padding)
    ...
    def apply_filter(self):
        ...
        # Nur zur History hinzufügen, wenn der Filterbegriff nicht leer ist und nicht bereits in der History
        if filter_text and filter_text not in self.filter_history:
            self.filter_history.append(filter_text)
            # Aktualisiere die Werte der Combobox
            self.filter_combo['values'] = list(reversed(self.filter_history))  # Neuester Eintrag oben
        self.filtered_images = []
        ...
        # Behalte den aktuellen Index bei, wenn das aktuelle Bild in der gefilterten Liste ist
    ...
    def on_folder_loaded(self, folder, file_path):
        if self.folder_images:
            # Normalisiere den file_path, falls vorhanden
            if file_path:
            ...
            # Prüfe, ob das ausgewählte Bild in folder_images ist
            ...
            # Wende den Filter an, nachdem das ausgewählte Bild angezeigt wurde
            self.apply_filter()
