...
- Sortieroption hinzugefügt:
  - Checkbox "Neuestes Bild zuerst" hinzugefügt, um zwischen ältestem und neuestem Bild zu wählen.
"""
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def __init__(self):
        ...
        self.sort_newest_first_var = tk.BooleanVar(value=True)  # Neue Checkbox für Sortierung
        self.preview_images = {}
    ...
    def setup_ui(self):
        ...
        self.sort_newest_first_cb = tk.Checkbutton(subfolder_frame, text="Neuestes Bild zuerst", variable=self.sort_newest_first_var,
                                                   fg=TEXT_FG_COLOR, bg=BG_COLOR, selectcolor=BG_COLOR, font=("Arial", self.main_font_size))
        self.sort_newest_first_cb.pack(side="left", padx=self.button_padding)
    ...
    def update_ui(self):
        ...
        if hasattr(self, 'sort_newest_first_cb'):
            self.sort_newest_first_cb.config(font=("Arial", self.main_font_size))
    ...
    def load_folder_async(self, folder, file_path=None):
        ...
        # Sortiere Bilder nach Erstellungsdatum
        self.folder_images.sort(key=lambda x: os.path.getctime(x), reverse=self.sort_newest_first_var.get())
