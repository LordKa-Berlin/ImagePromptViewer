...
def extract_text_chunks(img_path):
    ...
    log_path = os.path.join(os.path.dirname(__file__), "extract_debug_log.txt")
    def write_log(lines):
        with open(log_path, "a", encoding="utf-8") as log:
            log.write("\n" + "="*60 + "\n")
            log.write(f"Log für: {img_path}\n")
            log.write(f"Zeit: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            log.write("\n".join(lines))
            log.write("\n")
    ...
    full_text = ""
    param_key = None
    debug_info = [f"Debug: Bild geladen: {os.path.basename(img_path)}"]
    ...
    # === Logik 3: ComfyUI Marker ===
    try:
        MARKER_START = '"inputs":{"text":"'
        MARKER_END = '"parser":'
        pattern = re.escape(MARKER_START) + r'(.*?)' + re.escape(MARKER_END)
        matches = re.findall(pattern, original_text, flags=re.DOTALL)
        debug_info.append(f"Debug: ComfyUI Marker gefunden: {len(matches)} Treffer")
        if matches:
            prompt = matches[0] if len(matches) >= 1 else ""
            negativ = matches[1] if len(matches) >= 2 else ""
            if prompt or negativ:
                debug_info.append("Debug: USING ComfyUI marker extraction (Logik 3)")
                success = True
    except Exception as e:
    ...
    if not success:
        try:
            ...
            if idx_steps != -1:
                settings_text = normalized[idx_steps:]
            else:
                settings_text = str(data_dict.get("steps", ""))
            if prompt_json or negative_json:
                prompt = str(prompt_json)
                negativ = str(negative_json)
                settings = settings_text
                debug_info.append("Debug: USING JSON parsing (Logik 1)")
                success = True
        except Exception as e:
            debug_info.append(f"Debug: JSON parsing failed: {e}")

    # === Logik 2: Direkte Marker im String ===
    if not success:
        ...
        if ('"prompt":' in normalized_lower and 
            ...
            prompt = normalized[idx_prompt_new + len('"prompt":'): idx_negative_new].strip().strip('",')
            negativ = normalized[idx_negative_new + len('"negativeprompt":'): idx_steps_new].strip().strip('",')
            settings = normalized[idx_steps_new + len('"steps":'):].strip().strip('",')
            debug_info.append("Debug: USING New Markers extraction (Logik 2)")
            success = True

    # === Logik 5: Smart JPEG Marker Parsing ===
    if not success:
        try:
            def clean_textblock(text):
                return text.replace("safe_pos,", "").replace("safe_neg,", "").strip()

            def normalize_for_smart_parsing(text):
                text = text.replace("\x00", "")
                text = re.sub(r'(?<=\w) (?=\w)', '', text)
                text = re.sub(r'\\u0022', '"', text)
                return text

            normalized_text = normalize_for_smart_parsing(full_text)
            marker1 = 'prompt":"'
            marker2 = 'negativePrompt":"'
            marker3 = '"steps"'

            idx1 = normalized_text.find(marker1)
            idx2 = normalized_text.find(marker2)
            idx3 = normalized_text.find(marker3)

            prompt_block = clean_textblock(normalized_text[idx1 + len(marker1): idx2]) if idx1 != -1 and idx2 != -1 else ""
            negative_block = clean_textblock(normalized_text[idx2 + len(marker2): idx3]) if idx2 != -1 and idx3 != -1 else ""
            settings_block = normalized_text[idx3:].strip() if idx3 != -1 else ""

            if prompt_block or negative_block:
                prompt = prompt_block
                negativ = negative_block
                settings = settings_block
                debug_info.append("Debug: USING Logik 5 (Smart JPEG Fallback)")
                success = True
            else:
                debug_info.append("Debug: Logik 5 Marker nicht gefunden oder kein Inhalt extrahiert.")
        except Exception as e:
            debug_info.append(f"Debug: Logik 5 Fehler: {e}")

    if not success:
        debug_info.append("Debug: Keine der Logiken 1-5 konnte greifen. Rückgabe leer.")
...
class ImageManagerForm(TkinterDnD.Tk):
    ...
    def update_fs_texts(self):
        ...
        tk.Button(self.fs_text_frame, text="copy Prompt",
                  ...
                  bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=("Arial", self.main_font_size)
                  ).grid(row=1, column=0, padx=self.button_padding)
        ...
        tk.Button(self.fs_text_frame, text="copy Negative",
                  ...
                  bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=("Arial", self.main_font_size)
                  ).grid(row=1, column=1, padx=self.button_padding)
        ...
        tk.Button(self.fs_text_frame, text="copy Settings",
                  ...
                  bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=("Arial", self.main_font_size)
                  ).grid(row=3, column=0, columnspan=2, padx=self.button_padding)
        for i in range(2):
