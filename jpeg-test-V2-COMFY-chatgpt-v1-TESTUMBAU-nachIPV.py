...
def extract_prompt_data(img_path):
    """
    ...
    global last_full_block

    ##debug_lines = []
    ##debug_lines.append(f"Version: {VERSION}")
    ##debug_lines.append(f"Dateipfad: {img_path}")
...
def open_image():
    img_path = filedialog.askopenfilename(filetypes=[("JPEG Dateien", "*.jpg;*.jpeg")])
    if not img_path:
        return

    prompt, negative, settings = extract_prompt_data(img_path)
    ##?if not prompt and not negative and not settings:
        ##? eventuel als Info Zeile mit übergeben! messagebox.showwarning("Keine Daten gefunden", "Es konnten keine passenden Textsegmente extrahiert werden.")
        ##?return
  ...
  ##  if not last_debug_info:
    ...
    ##def save_debug_to_file():
      ##  timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        ##img_path_debug = os.path.join(os.path.dirname(__file__), f"debug_log_{timestamp}.txt")
        ##try:
          ##  with open(img_path_debug, "w", encoding="utf-8") as f:
