...
"""
Datum: 2025-03-25
Versionsnummer: 1.0.27
Interne Bezeichnung: Master 6
...
Änderungen in Version 1.0.27:
- Hinzufügung von Mausrad-Navigation im Hauptfenster: Scrollen vor/zurück über dem Bild mit dem Mausrad.
"""
...
# Automatische Installation fehlender Module
try:
...
def extract_png_text_chunks(png_path):
    try:
        ...
        return "", "", ""
    info = img.info
    full_text = info.get("parameters", "")
    if not full_text:
        full_text = " ".join([str(v) for v in info.values()])
    lower_text = full_text.lower()
    idx_params = lower_text.find("parameters")
    idx_neg = lower_text.find("negative prompt:")
    idx_steps = lower_text.find("steps:")
    prompt = full_text[idx_params+len("parameters"):idx_neg].strip()
    negativ = full_text[idx_neg+len("negative prompt:"):idx_steps].strip()
    settings = full_text[idx_steps:].strip()
...
class ImageManagerForm(TkinterDnD.Tk):
    def __init__(self):
        ...
        # Monitore ermitteln und primären Monitor auswählen
        ...
        self.image_info_label.pack(side="top", anchor="n", pady=self.button_padding)

        # Button "Bild öffnen"
        self.open_image_button = tk.Button(self, text="Bild öffnen", command=self.open_image_in_system,
                                           bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=("Arial", self.main_font_size))
        self.open_image_button.pack(anchor="n", padx=self.button_padding, pady=self.button_padding)
        ...
        tk.Button(folder_frame, text="Bild auswählen", command=self.select_image_from_folder,
                  bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=("Arial", self.main_font_size)).pack(side="left", padx=self.button_padding)
        tk.Button(folder_frame, text="Bild löschen", command=self.delete_current_image,
                  bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=("Arial", self.main_font_size)).pack(side="right", padx=self.button_padding)
        ...
        self.image_frame.pack(fill="both", padx=self.button_padding, pady=self.button_padding)
        ...
        self.image_label.bind("<MouseWheel>", self.on_image_mousewheel)  # Mausrad-Event hinzugefügt
        ...
        tk.Button(nav_frame, text="zurück", command=self.show_previous_image,
                  bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=("Arial", self.main_font_size), width=10).pack(side="left", padx=self.button_padding)
        tk.Button(nav_frame, text="weiter", command=self.show_next_image,
                  bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=("Arial", self.main_font_size), width=10).pack(side="left", padx=self.button_padding)
        ...
        self.scale_dropdown.configure(bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=("Arial", self.main_font_size + 2))  # Schriftgröße erhöht
        ...
        if len(self.monitor_list) > 1:
            ...
            self.monitor_menu.configure(bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=("Arial", self.main_font_size + 2))  # Schriftgröße erhöht
    ...
    def on_image_mousewheel(self, event):
        """Handler für Mausrad über dem Bild im Hauptfenster."""
        if event.delta > 0:
            self.show_previous_image()  # Hochscrollen -> vorheriges Bild
        elif event.delta < 0:
            self.show_next_image()      # Runterscrollen -> nächstes Bild

    def handle_drop(self, event):
        file_path = event.data.strip("{}")
        if os.path.isfile(file_path):
            self.display_image(file_path, default_scale=True)
            self.extract_and_display_text_chunks(file_path)
    ...
    def choose_folder(self):
        ...
        if folder:
            ...
            if self.search_subfolders_var.get():
                self.folder_images = self.recursive_list_png(folder)
            else:
            ...
            if self.folder_images:
                ...
                self.display_image(self.folder_images[self.current_index])
    ...
    def select_image_from_folder(self):
        ...
        if file_path:
            ...
            if self.search_subfolders_var.get():
                ...
                self.folder_images = [os.path.join(folder, f) for f in os.listdir(folder) if f.lower().endswith(".png")]
            self.folder_images.sort()
            if file_path in self.folder_images:
                self.current_index = self.folder_images.index(file_path)
            else:
                self.folder_images.insert(0, file_path)
                self.current_index = 0
            self.display_image(file_path)
            self.extract_and_display_text_chunks(file_path)
            self.populate_preview_table()

    def recursive_list_png(self, folder):
        png_files = []
    ...
    def display_image(self, file_path, default_scale=False):
        ...
        info_text = f"Dateiname: {os.path.basename(file_path)}    Pfad: {file_path}    Erstellt: {created_str}"
    ...
    def populate_preview_table(self):
        for widget in self.preview_inner_frame.winfo_children():
            widget.destroy()
        for i, file_path in enumerate(self.folder_images):
            try:
            ...
            tk.Label(frame, text=fname, fg=TEXT_FG_COLOR, bg=BG_COLOR, font=("Arial", self.main_font_size)).pack(side="left", padx=self.button_padding)
            link = tk.Label(frame, text=file_path, fg="blue", bg=BG_COLOR, font=("Arial", self.main_font_size), cursor="hand2")
            link.pack(side="left", padx=self.button_padding)
            link.bind("<Button-1>", lambda e, path=file_path: self.open_file_explorer(path))
    ...
    def delete_current_image(self):
        if hasattr(self, "current_image_path") and self.current_image_path:
            confirm = messagebox.askyesno("Bild löschen", f"Möchten Sie das Bild '{os.path.basename(self.current_image_path)}' löschen?")
            if confirm:
                try:
                    os.remove(self.current_image_path)
                    self.status("Bild gelöscht.")
    ...
    def show_fullscreen(self):
        if not hasattr(self, "current_image") or not self.current_image:
            ...
            return
        self.fs_image_path = self.folder_images[self.current_index]
        try:
        ...
        self.fullscreen_win.bind("<MouseWheel>", self.fullscreen_mousewheel)
        self.fs_text_focus = False

        # Bildinfo im Vollbild (10% kleiner)
        ...
        btn_fs_open = tk.Button(top_buttons, text="Bild öffnen", command=self.open_image_fs,
                                bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=("Arial", self.main_font_size))
    ...
    def fullscreen_mousewheel(self, event):
        if getattr(self, 'fs_text_focus', False):
            return "break"
        try:
            if event.state & 0x0004:  # Strg + Mausrad
                self.fullscreen_zoom(event)
            else:
                if event.delta > 0:
    ...
    def set_fs_text_focus(self, state):
        self.fs_text_focus = state
    ...
    def fs_delete_current_image(self):
        if hasattr(self, "fs_image_path") and self.fs_image_path:
            confirm = messagebox.askyesno("Bild löschen", f"Möchten Sie das Bild '{os.path.basename(self.fs_image_path)}' löschen?")
            if confirm:
                try:
                    os.remove(self.fs_image_path)
                    self.status("Bild gelöscht.")
                    self.fs_show_next()
                except Exception as e:
                    self.status(f"Fehler beim Löschen: {e}")
    ...
    def update_fs_texts(self):
        for widget in self.fs_text_frame.winfo_children():
            widget.destroy()
        p, n, s = extract_png_text_chunks(self.fs_image_path)
        desired_settings_lines = 4
        font_size = get_font_size(self.selected_monitor)
        custom_font = ("Arial", font_size)
        bold_font = ("Arial", font_size + 2, "bold")

        self.fs_text_frame.grid_rowconfigure(0, weight=1, minsize=int(100 * self.scaling_factor))
        self.fs_text_frame.grid_rowconfigure(1, weight=1, minsize=int(100 * self.scaling_factor))
        self.fs_text_frame.grid_rowconfigure(2, weight=0, minsize=desired_settings_lines * (font_size + 4))
        self.fs_text_frame.grid_columnconfigure(0, weight=1)

        # Prompt-Feld
        prompt_frame = tk.Frame(self.fs_text_frame, bg=BG_COLOR)
        prompt_frame.grid(row=0, column=0, sticky="nsew", padx=self.button_padding, pady=self.button_padding)
        tk.Label(prompt_frame, text="Prompt:", fg=TEXT_FG_COLOR, bg=BG_COLOR, font=bold_font).grid(row=0, column=0, sticky="w")
        fs_prompt = ScrolledText(prompt_frame, bg=TEXT_BG_COLOR, fg=TEXT_FG_COLOR,
                                 font=custom_font, wrap="word", takefocus=True)
        fs_prompt.grid(row=1, column=0, sticky="nsew")
        prompt_frame.grid_rowconfigure(1, weight=1)
        prompt_frame.grid_columnconfigure(0, weight=1)
        fs_prompt.insert(tk.END, p)
        fs_prompt.bind("<Enter>", lambda e: self.set_fs_text_focus(True))
        fs_prompt.bind("<Leave>", lambda e: self.set_fs_text_focus(False))
        fs_prompt.bind("<MouseWheel>", lambda e: fs_prompt.yview_scroll(-1 if e.delta > 0 else 1, "units"))
        tk.Button(prompt_frame, text="copy Prompt",
                  command=lambda: copy_to_clipboard(self, fs_prompt.get("1.0", tk.END)),
                  bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=custom_font).grid(row=2, column=0, sticky="e", pady=2)

        # Negativ-Feld
        negativ_frame = tk.Frame(self.fs_text_frame, bg=BG_COLOR)
        negativ_frame.grid(row=1, column=0, sticky="nsew", padx=self.button_padding, pady=self.button_padding)
        tk.Label(negativ_frame, text="Negativ Prompt:", fg=TEXT_FG_COLOR, bg=BG_COLOR, font=bold_font).grid(row=0, column=0, sticky="w")
        fs_negativ = ScrolledText(negativ_frame, bg=TEXT_BG_COLOR, fg=TEXT_FG_COLOR,
                                  font=custom_font, wrap="word", takefocus=True)
        fs_negativ.grid(row=1, column=0, sticky="nsew")
        negativ_frame.grid_rowconfigure(1, weight=1)
        negativ_frame.grid_columnconfigure(0, weight=1)
        fs_negativ.insert(tk.END, n)
        fs_negativ.bind("<Enter>", lambda e: self.set_fs_text_focus(True))
        fs_negativ.bind("<Leave>", lambda e: self.set_fs_text_focus(False))
        fs_negativ.bind("<MouseWheel>", lambda e: fs_negativ.yview_scroll(-1 if e.delta > 0 else 1, "units"))
        tk.Button(negativ_frame, text="copy Negativ",
                  command=lambda: copy_to_clipboard(self, fs_negativ.get("1.0", tk.END)),
                  bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=custom_font).grid(row=2, column=0, sticky="e", pady=2)

        # Settings-Feld
        settings_frame = tk.Frame(self.fs_text_frame, bg=BG_COLOR)
        settings_frame.grid(row=2, column=0, sticky="nsew", padx=self.button_padding, pady=self.button_padding)
        tk.Label(settings_frame, text="Settings:", fg=TEXT_FG_COLOR, bg=BG_COLOR, font=bold_font).grid(row=0, column=0, sticky="w")
        fs_settings = ScrolledText(settings_frame, height=desired_settings_lines, bg=TEXT_BG_COLOR, fg=TEXT_FG_COLOR,
                                   font=custom_font, wrap="word", takefocus=True)
        fs_settings.grid(row=1, column=0, sticky="nsew")
        settings_frame.grid_rowconfigure(1, weight=0)
        settings_frame.grid_columnconfigure(0, weight=1)
        fs_settings.insert(tk.END, s)
        fs_settings.bind("<Enter>", lambda e: self.set_fs_text_focus(True))
        fs_settings.bind("<Leave>", lambda e: self.set_fs_text_focus(False))
        fs_settings.bind("<MouseWheel>", lambda e: fs_settings.yview_scroll(-1 if e.delta > 0 else 1, "units"))
        tk.Button(settings_frame, text="copy Settings",
                  command=lambda: copy_to_clipboard(self, fs_settings.get("1.0", tk.END)),
                  bg=BTN_BG_COLOR, fg=BTN_FG_COLOR, font=custom_font).grid(row=2, column=0, sticky="e", pady=2)
